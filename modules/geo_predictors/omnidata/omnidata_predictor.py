...
#        ckpt_path = './checkpoints/omnidata_dpt_depth_v2.ckpt'
    ...
    def predict_depth(self, img, **kwargs):
        ...
        with torch.no_grad():
            ...
            if img.dim() == 3:
                img = img.unsqueeze(0)
            img_tensor = torch.nn.functional.interpolate(img, size=(self.img_size, self.img_size), mode='bilinear', align_corners=False)
            img_tensor = (img_tensor - 0.5) / 0.5
            output = self.model(img_tensor).clip(0., 1.)  # [B, 384, 384] or model head produces [B, 384, 384]
            output = torch.nn.functional.interpolate(output[:, None], size=(512, 512), mode='bicubic').clip(0., 1.)
        self.model.to(torch.device('cpu'))
        return output
