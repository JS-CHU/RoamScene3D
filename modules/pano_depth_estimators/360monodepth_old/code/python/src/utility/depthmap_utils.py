...
from modules.pano_depth_estimators.Depth_Anything_V2.depth_anything_v2.dpt import DepthAnythingV2
...
def Depth_Anything_prediction(rgb_image_data_list):
    ...
    # 允许用环境变量切换编码器与权重路径；默认保持与原实现一致
    import os
    encoder = os.getenv('DA_ENCODER', 'vitl')
    weights_default = f'checkpoint/depth_anything_v2_{encoder}.pth'
    weights_path = os.getenv('DA_WEIGHTS', weights_default)
