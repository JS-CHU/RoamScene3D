...
class StopWatch:
    ...
    def __call__(self, msg):
        ...
        print(f"{msg} {end - self.start:.2f}s")
        self.start = end
...
def main(args, enable_xformers_memory_efficient_attention=True,):
    ...
    if accelerator.is_main_process:
        ...
        watch = StopWatch()
        index = 0
        for image_name in tqdm(image_names, desc="PanoSR Progress"):
            ...
            resize_flag = True #
            # validation_prompt = 'A cozy, vintage-style bedroom with patterned wallpaper and drapes, a large white bed centered on a plush rug, an antique armoire, and upholstered armchairs under warm, soft lighting.'
            image = pipeline(
            ...
            if args.control_type=='grayscale':
                ...
                if not args.prefix == 'inpainted': 
                    w, h = image.size
                    image = image.resize((w // 2, h // 2), Image.LANCZOS)
                    image.save(f'{output_path}/{args.prefix}.png')
                else:
                    image.save(f'{output_path}/sr_{args.upscale}_{name}.png')
                # if index == num_layer:
                #     output_path = f"{base_dir}/{image_name}.png"
                # else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prefix", type=str, default="inpainted", help="prefix of image name")

    parser.add_argument("--inputs_dir", type=str, default="", help="layerdata dir")
    ...
    parser.add_argument("--latent_tiled_overlap", type=int, default=8, help="unet latent overlap size for saving GPU memory") # for 24G
